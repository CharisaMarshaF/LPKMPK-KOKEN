...
<div class="text-left mt-8">
    <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#header-footer-modal-preview"
       class="btn btn-primary">Tambah Dokumentasi</a>
</div>
...
<div class="intro-y grid grid-cols-12 gap-6 mt-5">
    <?php foreach ($documentation as $cc) { ?>
    <div class="intro-y col-span-12 md:col-span-6 xl:col-span-4 box">
        <!-- Gambar -->
        <div class="p-5">
            <div class="h-40 2xl:h-56 image-fit">
                <img alt="Galeri" class="rounded-md object-cover w-full h-full"
                     src="<?= base_url('assets/upload/dokumentasi/' . $cc['foto']) ?>">
            </div>

            <!-- Judul -->
            <div class="mt-3 text-base font-medium truncate text-center">
                <?= htmlspecialchars($cc['judul']) ?>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">TAMBAH DOKUMENTASI</h2>
            </div>
            <form action="<?= base_url('admin/dokumentasi/simpan') ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12">
                        ...
                        <input name="judul" id="modal-form-nama" type="text" class="form-control" placeholder="Judul Dokumentasi" required>
                    </div>
                    <div class="col-span-12">
                        <label class="form-label">Upload Foto Dokumentasi (1 : 3)</label>
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "<?= base_url('admin/dokumentasi/delete_data/') ?>" + foto;
            }
        });
    });
</script>
